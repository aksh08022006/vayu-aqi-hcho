...
> DBSCAN, the vectorised AQI grid) are unit-tested.
...
- Detection: PHV (flagship), **Getis-Ord Gi\*** (z > 1.96/2.58/3.29; + FDR), DBSCAN,
  P95. Mitigate single-pixel artefacts with composites + persistence + significance.
...
Objective 1 (AQI):
  INSAT AOD + TROPOMI columns + ERA5 met + landcover/elevation/fire
    -> preprocessing (regrid, QA, collocate) -> features -> CNN-LSTM (+RF/XGB)
    -> surface PM2.5/PM10/NO2/SO2/CO/O3 -> CPCB AQI engine -> daily/seasonal AQI maps
  Ground truth: CPCB CAAQMS stations.

Objective 2 (HCHO):
  TROPOMI HCHO + VIIRS/MODIS fire + ERA5 winds + landcover
    -> seasonal composites -> hotspots (PHV/Gi*/DBSCAN/P95)
    -> source attribution -> HCHO-O3 (FNR) -> wind back-trajectory transport
    -> HCHO hotspot atlas.
```
...
| Features | `features/engineering.py` | ✅ FNR, lags, interactions, cyclical time |
| Models | `models/{baselines,cnn,lstm,cnn_lstm,dataset,train}.py` | ✅ trained in demo |
| AQI | `aqi/engine.py` | ✅ fully implemented + unit-tested (scalar + vectorised) |
| HCHO | `hcho/{phv,getis_ord,dbscan_hotspots,percentile,source_attribution,ozone_relationship,transport}.py` | ✅ implemented + tested |
| Explainability | `explain/shap_analysis.py` | ✅ run in demo |
...
(block) R² **−0.10**, temporal R² **0.86**. Spatial ≪ random because the data
embeds an unobserved per-station emission factor — so leave-station-out / block CV
is the honest metric (Wang 2023).

**AQI (peak burning day, 2021-11-13):** mean 159, max 411; cells span Good →
Severe (Moderate 1503, Satisfactory 1058, Poor 895, Very Poor 196, Severe 1).

**HCHO hotspots (burning-window composite):** PHV flags 2.8 % of cells (102 local
anomalies); Getis-Ord Gi* 1047 significant cells; DBSCAN groups the PHV anomalies
into **15 clusters**, attributed as `agri_burning` (Punjab/Haryana, FRP≈215),
`urban` (Mumbai), `industrial` (Korba/Jharkhand/Gujarat), `biogenic`, `other`.

**HCHO–O₃:** Pearson r = **0.42**; FNR regimes VOC-limited 5694 / transition 655 /
NOₓ-limited 851 cells.

**Transport:** Delhi 48 h back-trajectory passes within 150 km of **741** fire
pixels — a clear upwind biomass-burning → receptor link.

**SHAP (PM2.5 drivers):** AOD dominates (mean |SHAP| ≈ 24.4), then NO₂,
precipitation, BLH — the physically expected hierarchy.
...
- **SHAP ranks AOD as the dominant PM2.5 driver**, with BLH, precipitation and NO₂
  next — the physically expected hierarchy.
- **HCHO–O₃ correlation is positive and rises at a multi-day lag** — HCHO behaving
  as an ozone precursor, as in Dong 2026.
- **PHV / Gi* / DBSCAN all flag the Punjab–Haryana burning belt**, and source
  attribution labels them as agricultural-burning in the post-monsoon window.

Artifacts produced in `outputs/`:
- `maps/` — daily + seasonal-mean AQI, predicted PM2.5, HCHO + hotspots, fire density
- `figures/` — HCHO–O₃ scatter, Delhi wind rose, SHAP importance
- `*.csv` — attributed hotspots, Delhi back-trajectory, SHAP importances
...
make transport                 # HCHO-O3 + wind transport
...
| Obj-2 hotspot accuracy/clarity | PHV/Gi*/DBSCAN/P95 + composites + significance/FDR |
| Multi-source integration | unified database (8 datasets co-registered) |
| Scientific interpretation | HCHO–O₃ FNR regimes, SHAP, transport attribution |
