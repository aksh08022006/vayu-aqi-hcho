...
4. **DBSCAN**: cluster cells above a threshold into contiguous clusters with centroids.
5. **P95**: flag cells above the 95th percentile (baseline).
6. Overlay all four; export hotspot tables for attribution.
...
| DBSCAN | cluster | Groups cells → centroids for attribution; drops noise | Needs eps/minPts tuning; threshold-dependent | Convert flagged cells into source objects |
| P95 | cell | Trivial, reproducible baseline | Absolute, no spatial context | Quick sanity mask |

### Why PHV at 1 km is the flagship
Dong et al. 2026 [A] find 0.01° (~1 km) optimal: 0.5 km over-fits TROPOMI retrieval noise, 2 km smooths real anomalies away. The **mutation/change-detection** refinement is decisive — it cut HVA candidates **60,042 → 4,431 (−92.6%)** and raised factory/HVA query accuracy **5.38% → 41.32%**, with per-year PHV% ≈ 5–13%. PHV is thus both selective and attributable. Gi*/DBSCAN/P95 are methodological extensions/baselines, not paper methods.
...
```
DBSCAN (extension): core point if ≥ `minPts` neighbours within `eps` (degrees); reachable cores merge into clusters; label −1 = noise.

## Python libraries
`xarray`, `numpy`, `pandas`; `esda` + `libpysal` (Gi*, DistanceBand); `scikit-learn` (DBSCAN). Tested with `pytest`.

## Code in this repo
`src/isro_aqi/hcho/phv.py` (`phv_field`, `detect_hotspots` → `phv`/`hva`/`hva_confirmed`, `phv_percent`), `getis_ord.py` (`gi_star`, `summarise`, `_benjamini_hochberg`), `dbscan_hotspots.py` (`cluster_hotspots`), `percentile.py` (`percentile_threshold`). Tests: `test_phv.py`, `test_getis_ord.py`, `test_dbscan.py`. Driver: `pipelines/06_hcho_analysis.py`.
...
- Gi* z/p/hotspot rasters (FDR-corrected); DBSCAN cluster table (centroid, n_cells, hcho_mean); P95 mask.
